...
var SYS_WITH_TRANSCRIPT = `You de-clickbait YouTube titles. You have the original title, the channel name, and a transcript excerpt of what the video actually contains.
...
async function fetchTranscript(videoId) {
  const RETRYABLE = /* @__PURE__ */ new Set([408, 429, 503]);
  for (let attempt = 0; attempt < 4; attempt++) {
    if (attempt > 0) await new Promise((r2) => setTimeout(r2, 2 ** attempt * 1e3));
    ...
    try {
      ...
      text = Array.isArray(parsed) ? parsed.map((s2) => s2.text ?? "").join(" ") : parsed.text ?? parsed.content ?? raw;
    } catch {
      text = raw;
    }
    if (!text.trim()) return null;
    return text.split(/\s+/).slice(0, 3e3).join(" ");
  }
  return null;
}
var handler = async (event) => {
  ...
  for (const record of event.Records) {
    const { videoId, originalTitle, creator } = JSON.parse(record.body);
    try {
      const transcript = await fetchTranscript(videoId);
      const system = transcript ? SYS_WITH_TRANSCRIPT : SYS_TITLE_ONLY;
      const userMsg = transcript ? `Creator: ${creator}
Original title: ${originalTitle}

Transcript excerpt:
${transcript}` : `Creator: ${creator}
Original title: ${originalTitle}`;
